...
pub fn combat(ecs: &mut SubWorld, commands: &mut CommandBuffer) {
    ...
    let defenders: Vec<(Entity, Entity, Entity)> = attackers
        .iter(ecs)
        .map(|(entity, attack)| (*entity, attack.attacker, attack.defender))
        .collect();

    defenders.iter().for_each(|(message, attacker, defender)| {
        let is_player = ecs
            .entry_ref(*defender)
            .unwrap()
